<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            @if(request()->routeIs('blog.list_newest'))
                {{ __('Newest blogs') }}
            @elseif(request()->routeIs('blog.own_list'))
                {{ __('Your blogs') }}
            @endif
        </h2>
        @if(request()->routeIs('blog.list_newest'))
            <form class="mt-2" action="{{ route('blog.list_newest') }}" method="get">
                <label for="search">Search</label>
                <input value="{{ request()->get('search') ?? '' }}" class="border-solid rounded border-gray-200"
                       type="text" id="search" name="search">
                <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                    Submit
                </button>
                <a class=" text-gray-400 font-bold ml-2 py-2 px-4 rounded border-solid border-2 border-gray-400"
                   href="{{ route('blog.list_newest') }}">Clear</a>
            </form>
                ...
                @if(request()->routeIs('blog.own_list'))
                    <div class="p-6 text-gray-900">
                        <a class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
                           href="{{ route('blog.create')}}">Write new</a>
                    </div>
                @endif
                                        ...
                                        @if(request()->routeIs('blog.own_list'))
                                            <div class="icons w-16 flex justify-between justify-items-center">
                                                <a href="{{ route('blog.edit', ['post' => $post]) }}">
                                                    <svg class="max-h-5 max-w-5" xmlns="http://www.w3.org/2000/svg"
                                                ...
                                                <a href="{{ route('blog.delete', ['post' => $post]) }}"
